...
HemicubeCorrector::HemicubeCorrector(GLuint width) {
  this->topData = std::vector<GLfloat>(width * width, 0.0);
  ...
  for (GLint ix = 0; ix < width; ix++) {
    for (GLint iy = 0; iy < width; iy++) {
      ...
      this->topData[ix * width + iy] = term;
    }
  }

  this->sideData = std::vector<GLfloat>(width * halfWidth, 0.0);
  for (GLint iy = 0; iy < width; iy++) {
