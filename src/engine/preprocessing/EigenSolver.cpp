...
void EigenSolver::init(GLuint size, std::vector<std::tuple<GLuint, GLuint, GLfloat>> triplets) {
...
std::vector<GLfloat> EigenSolver::solve(std::vector<GLfloat> emissions) {
	...
	// Get emissions
	Eigen::Map<Eigen::VectorXf, Eigen::Unaligned> emissionsXf(emissions.data(), emissions.size());
	...
	std::vector<GLfloat> radiosity = std::vector<GLfloat>(size_t(_radiosity.size()), 0.0f);
