...
Shader::Shader(std::string vertName, std::string geomName,
               std::string fragName) {
  ...
  EngineStore::logger.log("Loading vertex shader " + vertName);
  loadShader(GL_VERTEX_SHADER, GLVertexId, base + vertName);
  EngineStore::logger.log("Loading geometry shader " + geomName);
  loadShader(GL_GEOMETRY_SHADER, GLgeomId, base + geomName);
  EngineStore::logger.log("Loading fragment shader " + fragName);
...
void Shader::bind() { glUseProgram(GLProgramId); }

void Shader::unbind() { glUseProgram(0); }

GLuint Shader::getID() { return this->GLProgramId; }
...
Shader::~Shader() { glDeleteProgram(GLProgramId); }
